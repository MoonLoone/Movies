...
public class NetworkUtils {
    ...
    private static final String API_KEY= "e25904a3b02636309f822184a2403ceb";
    ...
    public static URL buildURLToReviews(int id, String lang){
        Uri uri = Uri.parse(String.format(BASE_URL_REVIEWS,id)).buildUpon().appendQueryParameter(PARAMS_API_KEY, API_KEY).appendQueryParameter(PARAMS_LANGUAGE,lang).build();
        try {
    ...
    public static URL buildURLToVideos(int id, String lang){
        Uri uri = Uri.parse(String.format(BASE_URL_VIDEOS,id)).buildUpon().appendQueryParameter(PARAMS_API_KEY, API_KEY).appendQueryParameter(PARAMS_LANGUAGE, lang).build();
        try {
            return new URL(uri.toString());
        } catch (MalformedURLException e) {
            e.printStackTrace();
    ...
    public static URL buildURL(int sortBy, int page, String lang){
        ...
        Uri uri = Uri.parse(BASE_URL).buildUpon()
                ...
                .appendQueryParameter(PARAMS_MIN_VOTE_COUNT,MIN_VOTE_COUNT_VALUE).build();
        try {
    ...
    //Класс для получения информации из интернета
